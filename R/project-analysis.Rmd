...
# LOO analysis
# All plots are created at the same time so I have to keep repeating code
# to individually change the text sizes (rolling eyes emoji)
...
Remove studies and get forest plot.

```{r}
meta_LOO <- update.meta(all_studies_meta, exclude = c(5, 15, 19))
save_base_plot(
  meta_LOO,
  here::here("figures", "loo-forest.png"),
  # I hate this solution but can't figure out a better way
  FUN = function(x, ...) {meta::forest.meta(x, sortvar = TE, ...)},
  fs.hetstat = 10
)
...
```{r}
meta_GOSH <- update.meta(all_studies_meta, exclude = c(5, 11, 14, 19))
save_base_plot(
  meta_GOSH,
  here::here("figures", "gosh-forest.png"),
  # I hate this solution but can't figure out a better way
  FUN = function(x, ...) {meta::forest.meta(x, sortvar = TE, ...)},
  fs.hetstat = 10
)
...
```{r}
no_outliers <- update.meta(all_studies_meta, exclude = c(5, 11, 14, 19))
save_base_plot(
  no_outliers,
...
Subgroup analysis for genotype.
...
gt_meta <- gt_df %>%
  meta::metaprop(
    ...
    hakn = TRUE,
    prediction = TRUE,
    title = "Proportion infected by inoculum genotype",
    subgroup = genotype
  )

save_base_plot(
  gt_meta,
  here::here("figures", "gt-forest.png"),
  FUN = function(x, ...) {meta::forest.meta(x, sortvar = TE, ...)},
  h = 3200,
  fs.hetstat = 10
)
...
Not enough info, so stratify by genogroup instead.
...
Risk of bias (low vs med/high).

```{r}
rob_df <- dat %>%
  mutate(
    RoB = ifelse(risk_of_bias == "H", "High", "Med/Low")
  ) %>%
  group_by(study, RoB) %>%
  summarize(
    across(starts_with("number"), sum),
...
save_base_plot(
  rob_meta,
  here::here("figures", "rob-forest.png"),
  FUN = function(x, ...) {meta::forest.meta(x, sortvar = TE, ...)},
  h = 2800,
  fs.hetstat = 10
)
...
ggsave(here::here("figures", "rob-summary.png"), width = 6.25, height = 3)
...
ggsave(here::here("figures", "rob-traffic-light.png"), width = 6, height = 8)
```
...
pth <- here::here("figures", "meta-reg.png")
pal <- hcl.colors(n = length(unique(dat_dose_es$study)),
                  palette = "Dark 2")
# Create graphics device
png(file = pth, width = 3200, height = 1800, res = 300)
# Actually generate forest plot
plot(x = dat_dose_es$log_dose,
     y = exp(dat_dose_es$yi) / (1 + exp(dat_dose_es$yi)),
     col = pal[dat_dose_es$study |> as.factor() |> as.integer()],
     xlab = "log dose",
     ylab = "proportion infected")
curve(exp(coef(dose_reg)[[1]] + coef(dose_reg)[[2]] * x) /
        (1+exp(coef(dose_reg)[[1]] + coef(dose_reg)[[2]] * x)),
      from = 0, to = 12, n = 10000, add = TRUE)
# Turn off graphics device (quietly)
zlib::quiet(dev.off())
# Trim whitespace
magick::image_read(pth) |>
...
```{r}
peters <- metabias(all_studies_meta, method.bias = "peters")
save_base_plot(
  all_studies_meta,
  here::here("figures", "peters-regression.png"),
  FUN = function(x) {
    with(x, {
      plot(
        x = 1 / all_studies_meta$n,
        y = all_studies_meta$TE,
        cex = 1 / sqrt(1/event + 1/(n - event)),
        xlab = "inverse sample size",
        ylab = "estimated log odds"
      )
      abline(a = peters$estimate["intercept"],
             b = peters$estimate["bias"],
             lty = 2)
    }
    )
  }
